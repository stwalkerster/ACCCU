...
for row in table:
    ...
    for item in ip:
        ...
        db.commit()
        cur.execute("INSERT INTO production.log (objectid, objecttype, user, action, timestamp) VALUES ("+str(row[0])+", \"Request\", 1733, \"Deferred to users\", \""+time.strftime('%Y-%m-%d %H:%M:%S')+"\");")
        db.commit()
